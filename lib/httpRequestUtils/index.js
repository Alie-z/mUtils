...
export default class HttpRequestUtils {
     ...
     */
    static init(fn) {
        LogUtils.logInfo('https://alie-z.github.io/mUtils/#/', `[mUtils] http_request HttpRequestUtils.init 需要自定义aixos的响应拦截以及基本配置 => `);
    ...
    static async get(url, params, config) {
        return new Promise((resolve, reject) => {
            if (!HttpRequestUtils.isInit) {
                LogUtils.logError('需要执行HttpRequestUtils.isInit()方法，才可以执行请求操作', `[mUtils] http_request: => error`);
            ...
            axios.get(url, getInfo).then((res) => {
                ...
                LogUtils.group(`[mUtils] http_request get 请求失败 ${url}`, LogUtils.errorColor);
    ...
    static post(url, data, config) {
        return new Promise((resolve, reject) => {
            ...
            axios.post(url, postInfo).then((res) => {
                LogUtils.group(`[mUtils] http_request post 请求成功 => ${url}`, LogUtils.successColor);
                LogUtils.logInfo(res.data, `http_request response => `);
                LogUtils.logInfo(res.config, `http_request config => `);
                LogUtils.logInfo(res.config.params, `http_request params => `);
                LogUtils.groupEnd();
                ...
                LogUtils.group(`[mUtils] http_request post 请求失败 => ${url}`, LogUtils.errorColor);
                LogUtils.logInfo(e, `http_request error => `);
                LogUtils.groupEnd();
                reject(e);
            });

 ...
 * @example
 * hasClass(document.body, 'mUtils')
 */
export function hasClass(el, className) {
    return el.classList.contains(className);
}
/**
 * @description 元素是添加class类
 * @param { HTMLElement } el dom元素
 * @param { String } className class名称
 * @example
 * hasClass(document.body, 'mUtils')
 ...
 * @example
 * removeClass(document.body, 'mUtils')
 */
...
export function computedStyle(el, cssProp) {
    if (!el) {
        LogUtils.logError('dom元素不存在', '[mUtils] DomUtils computedStyle error => ');
        return;
    }
    if (!cssProp) {
        LogUtils.logError('请输入需要查询的css属性名称', '[mUtils] DomUtils computedStyle error => ');
        return;
    }
    return document.defaultView.getComputedStyle ? document.defaultView.getComputedStyle(el, '')[cssProp] : el.currentStyle[cssProp];
}
/**
 * @description js设置元素的filter样式 (滤镜)
 * @param { HTMLElement } el dom元素
 ...
 * })
 */
export function cssFilter(el, type, option) {
